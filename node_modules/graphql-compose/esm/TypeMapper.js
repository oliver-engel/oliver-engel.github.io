...
export class TypeMapper {
  ...
  static isOutputType(type) {
    return type instanceof ScalarTypeComposer || type instanceof ObjectTypeComposer || type instanceof InterfaceTypeComposer || type instanceof UnionTypeComposer || type instanceof EnumTypeComposer || type instanceof NonNullComposer && TypeMapper.isOutputType(type.ofType) || type instanceof ListComposer && TypeMapper.isOutputType(type.ofType) || type instanceof ThunkComposer;
  }

  static isInputType(type) {
    return type instanceof ScalarTypeComposer || type instanceof EnumTypeComposer || type instanceof InputTypeComposer || type instanceof NonNullComposer && TypeMapper.isInputType(type.ofType) || type instanceof ListComposer && TypeMapper.isInputType(type.ofType) || type instanceof ThunkComposer;
  }
  ...
  convertOutputFieldConfig(composeFC, fieldName = '', typeName = '') {
    try {
      ...
      if (isFunction(composeFC)) {
        return createThunkedObjectProxy(() => this.convertOutputFieldConfig(composeFC(), fieldName, typeName));
  ...
  convertArgConfig(composeAC, argName = '', fieldName = '', typeName = '') {
    try {
      ...
      if (isFunction(composeAC)) {
        return createThunkedObjectProxy(() => this.convertArgConfig(composeAC(), argName, fieldName, typeName));
  ...
  convertInputFieldConfig(composeIFC, fieldName = '', typeName = '') {
    try {
      ...
      if (isFunction(composeIFC)) {
        return createThunkedObjectProxy(() => this.convertInputFieldConfig(composeIFC(), fieldName, typeName));
      } // convert type when its provided as composeIFC
